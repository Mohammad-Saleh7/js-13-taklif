...
function getNamesByLetter() {
  ...
  while (true) {
    letter = prompt("Enter a letter (a-z or A-Z):");
    if (letter === null) {
        return;  
    } else if (letter.length !== 1) {
        alert("Please enter only one alphabet character.");
    } else if (letter.toLowerCase() < 'a' || letter.toLowerCase() > 'z') {
  ...
  const filteredNames = names.filter(name => name.toLowerCase().startsWith(lowerCaseLetter));
  if (filteredNames.length > 0) {
    console.log(`Names starting with '${letter}':`, filteredNames.join(", "));
  } else {
